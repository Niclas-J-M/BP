...
from SMDP_single.manager import Manager
    ...
    for episodes in range(num_episodes):
        ...
        while True:
            ...
            if manager.options_in_state_space(region, task):
                ...
                #print("steps", steps)
                #print("total_steps_iteration", total_steps_iteration)
            ...
            if intended_transitions:
                intended_worker.train(intended_transitions)
                for _ in range(4):
                    intended_worker.train_sil()
            ...
            if goal_region > num_regions:
                goal_region = region
            # changed actual end region is an object end state
            if actual_end_region > num_regions and actual_end_region != 0:
                option = actual_end_region
                goal_region = region

            #print(task, region, option, reward, goal_region)
